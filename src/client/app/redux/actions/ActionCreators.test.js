import {
  setSearchFilter, setSortingType, beginGetFilms, beginGetFilmInfo, getFilmsSuccess, getFilmInfoSuccess, getFilmsError,
  getFilmInfoError,
} from './index';
...
describe('>>>A C T I O N --- Test ActionsCreators', () => {
  it('+++ actionCreator setSearchFilter', () => {
    ...
    expect(search).toEqual({ type: 'SET_SEARCH_FILTER', payload: {filter : filterValue } });
  });
  it('+++ actionCreator setSortingType', () => {
    const sortValue = 'release_date';
    const setSortType = setSortingType(sortValue);
    expect(setSortType).toEqual({ type: 'SET_SORTING_TYPE', payload: {sortType: sortValue } });
  });
  ...
  it('+++ actionCreator getFilmsSuccess', () => {
    ...
    expect(getMoviesSuccess).toEqual({ type: 'GET_FILMS_SUCCESS', payload: filmsMockArray });
  });
  it('+++ actionCreator getFilmInfoSuccess', () => {
    const getMovieSuccess = getFilmInfoSuccess(filmMock);
    expect(getMovieSuccess).toEqual({ type: 'GET_FILM_INFO_SUCCESS', payload: filmMock });
  });
